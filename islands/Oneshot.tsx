...
export default function App({ validators }: AppProps) {
  ...
  const setupLucid = async (e: Event) => {
    ...
    if (cache) {
      ...
      setTokenName(tokenName);// set token name
      setGiftADA(giftADA);// set ada amount
  ...
  const createGiftCard = async (e: Event) => {
    ...
    try {
      ...
      const assetName = `${parameterizedContracts!.policyId}${
        fromText(tokenName)// get the token name
      }`;
        ...
        : (
          ...
          >
            <Input
              ...
              id="tokenName"
              value={tokenName}
              onInput={(e) => setTokenName(e.currentTarget.value)}// set the token name
            >
              Token Name
            </Input>
